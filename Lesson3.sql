...
*/
SELECT a.name, a.id, SUM(o.total_amt_usd) total_spend
FROM orders o
JOIN accounts a
ON a.id=o.account_id
GROUP BY a.name, a.id
...
*/
SELECT a.name, a.id, w.channel, COUNT(*) Num_contacts
FROM web_events w
JOIN accounts a
ON a.id = w.account_id
...
10 orders, and include the id and account_id fields. NOTE - you will be thrown
an error with the correct solution to this question. This is for a division by
zero. You will learn how to get a solution without an error to this query when
...
which are sales reps associated with more than 200 orders or more than 750000 in
total sales. The middle group has any rep with more than 150 orders or 500000 in
